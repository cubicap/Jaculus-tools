...
program
    ...
    .option("--idf [path]", "Path to ESP-IDF [<path>, download, force-download]", "download")
    .option("--platform [platform]", "Platform to build for [" + platforms.join(", ") + "]", "esp32")
    .option("--force-download", "Force redownload of Jaculus")
...
program.action(async (options: OptionValues) => {
    ...
    if (!platforms.includes(options.platform)) {
        stderr.write(chalk.red("Invalid platform specified: " + options.platform + "\n"));
        process.exit(1);
    }

    if (options.idf == "force-download") {
        stdout.write(chalk.green("Removing existing ESP-IDF\n"));
        fs.rmSync(getIdfPath(), { recursive: true, force: true });
    }

    // ----- INSTALL ESP-IDF -----
    let idfPath = getIdfPath(options.idf);

    if (["download", "force-download"].includes(options.idf) && !fs.existsSync(idfPath)) {
    ...
    if (options.forceDownload && fs.existsSync(jacPath)) {
    ...
    // ----- CONFIGURE JACULUS -----
    stdout.write(chalk.green("\nConfiguring Jaculus for " + options.platform + "\n"));
    ...
    try {
        fs.copyFileSync(path.join(jacPath, "sdkconfig-" + options.platform), path.join(jacPath, "sdkconfig"));
    }
    catch (err) {
    ...
    let idfCommand = "idf.py -p " + options.port + " build flash";
